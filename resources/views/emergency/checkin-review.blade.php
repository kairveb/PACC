...
<div class="mx-auto max-w-5xl space-y-6">
    ...
    <div class="panel-card p-6">
        <dl class="grid gap-5 text-sm md:grid-cols-2">
            ...
            <div><dt class="text-slate-500">Phone</dt><dd class="font-medium text-slate-900">{{ $profile->phone ?? $profile->contact_phone ?? $patient->phone ?? 'Not provided' }}</dd></div>
            <div><dt class="text-slate-500">Email</dt><dd class="font-medium text-slate-900">{{ $profile->email ?? $profile->contact_email ?? $patient->email ?? 'Not provided' }}</dd></div>
            <div class="md:col-span-2"><dt class="text-slate-500">Address</dt><dd class="font-medium text-slate-900">{{ trim(implode(', ', array_filter([$profile->address_line1, $profile->address_barangay, $profile->address_city, $profile->address_province, $profile->address_postal ?? $profile->address_postal_code]))) ?: 'Not provided' }}</dd></div>
            ...
            <div class="md:col-span-2"><dt class="text-slate-500">Emergency contact</dt><dd class="font-medium text-slate-900">{{ trim(implode(' / ', array_filter([$profile->emergency_name ?? $profile->emergency_contact_name, $profile->emergency_relationship ?? $profile->emergency_contact_relationship, $profile->emergency_phone ?? $profile->emergency_contact_phone]))) ?: 'Not provided' }}</dd></div>
