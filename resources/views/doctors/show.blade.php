...
<div class="mx-auto max-w-6xl space-y-6">
    <div class="panel-card p-6">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <div class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Triage record</div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('doctors.queue') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">Back to queue</a>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-6">
            <div class="panel-card p-6">
                <div class="mb-4 flex items-center justify-between border-b border-slate-200 pb-4">
                    <h3 class="text-lg font-semibold text-slate-900">Clinical summary</h3>
                    <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-600">Review</span>
                </div>
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                        <div class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Pain score</div>
                        <div class="mt-2 text-base font-medium text-slate-800">{{ $assessment->pain_score ?? '—' }}/10</div>
                    </div>
                    <div class="md:col-span-2">
                        <div class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Symptoms</div>
                        <div class="mt-2 text-base leading-6 text-slate-700">
                    ...
                    <div class="md:col-span-2">
                        <div class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Nurse notes</div>
                        <div class="mt-2 text-base leading-6 text-slate-700">{{ $assessment->notes ?? 'No additional nurse notes recorded.' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="panel-card p-6">
                <div class="mb-4 flex items-center justify-between border-b border-slate-200 pb-4">
                    <h3 class="text-lg font-semibold text-slate-900">Vital signs</h3>
                    <span class="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-700">Current</span>
                </div>
                @if ($assessment->vitals)
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">BP</div>
                            <div class="mt-2 text-xl font-semibold text-slate-900">{{ $assessment->vitals->blood_pressure ?? '—' }}</div>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">HR</div>
                            <div class="mt-2 text-xl font-semibold text-slate-900">{{ $assessment->vitals->heart_rate ?? '—' }} bpm</div>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">RR</div>
                            <div class="mt-2 text-xl font-semibold text-slate-900">{{ $assessment->vitals->respiratory_rate ?? '—' }}/min</div>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Temp</div>
                            <div class="mt-2 text-xl font-semibold text-slate-900">{{ $assessment->vitals->temperature ?? '—' }}°C</div>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">SpO₂</div>
                            <div class="mt-2 text-xl font-semibold text-slate-900">{{ $assessment->vitals->spo2 ?? '—' }}%</div>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Weight</div>
        ...
        <aside class="space-y-6">
            <div class="panel-card p-6">
                <div class="mb-4 flex items-center justify-between border-b border-slate-200 pb-4">
                    ...
                    <span class="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-amber-700">AI</span>
                </div>
                <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <span class="status-pill {{ $pill }}">{{ $priorityLabel }}</span>
                        <span class="text-xs font-medium text-slate-500">Priority score: {{ $assessment->priority_score ?? '—' }}</span>
                    </div>
                    <p class="text-sm leading-6 text-slate-700">{{ $assessment->notes ?? 'No recommendation text was stored for this triage record.' }}</p>
                </div>
            </div>

            <div class="panel-card p-6">
                <div class="mb-4 flex items-center justify-between border-b border-slate-200 pb-4">
                    <h3 class="text-lg font-semibold text-slate-900">Consult status</h3>
                </div>
                <div class="space-y-4">
                    @php
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
                        Current status: <span class="status-pill {{ $consultStatusClass }}">{{ $consultStatusLabel }}</span>
                    </div>
            ...
            <div class="panel-card p-6">
                <div class="mb-4 flex items-center justify-between border-b border-slate-200 pb-4">
                    <h3 class="text-lg font-semibold text-slate-900">Clinical metadata</h3>
                </div>
